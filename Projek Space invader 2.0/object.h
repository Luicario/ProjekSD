...
enum EnemyType { BASIC_ENEMY, ZIGZAG_ENEMY, SEEKER_ENEMY, BOX};
...
class HealthBox : public CBasicEnemy
{
protected:
public:
	HealthBox(game* w, int hp = 1, float x = 0.0f, float y = 0.0f, float dx = 0.0f, float dy = ITEM_SPEED);
	void Draw();
	void Erase();
	void Update(float elapsed);
	EnemyType GetType();
