...
gotoxy(x, y+7); printf("         developed by OUR KELOMPOK. - 2020");
}
...
void DrawSpaceShipInfo(){ // Displays HP and energy points, I aligned them with the labels printed in DrawGameLimits
	...
	gotoxy(23, 1); printf("     ");
		...
		for(int i = 0; i < energy; i++){
			gotoxy(23 + i, 1); printf("O", 222);
		}
}

void Draw(){ // This is our spaceship
gotoxy(x,y + 1); printf( "  -  ", 4);
gotoxy(x,y + 2); //printf("(=|=)",17, 30, 223, 30, 16);
cout<<"(=|=)";
}
...
x = _x;
y = _y;
hp = 1; // I designed the game to have 1 lifes
imDead = false; // By default you are not dead
}

void DrawEnemy(){ // This is our spaceship
	gotoxy(x,y); //printf("|=O=|",17, 30, 223, 30, 16);
	cout<<"|=O=|";}

void EraseEnemy(){ // This was or spaceship
	gotoxy(x,y); //printf("     ");
	cout<<"     ";
}
};
//====================AKHIR CLASS Enemy COBA COBA 
...
for(int i = 0; i < 5; i++) { // Pick as many asteroids as you want
Asteroids.push_back(new Asteroid(rand()%78 + 1, rand()%4 + 3));// They are randomly placed in the map but not too low
}

SpaceShip ss = SpaceShip(40,20); // Here our adventure begins
Enemy ee = Enemy(20,10);
...
ee.DrawEnemy();//COBA ENEMY

for(bullet = Bullets.begin(); bullet != Bullets.end();){ // For every bullet that is in space
	(*bullet)->Move();
...
for(asteroid = Asteroids.begin(); asteroid != Asteroids.end(); ++asteroid){
	for(bullet = Bullets.begin(); bullet != Bullets.end();){ // asteroid-bullet collision
		...
		if((astX == bulX) && ((astY == bulY) || (astY + 1 == bulY))){ //Chances are that in the Y axis they never reach the same value, that case must be considered
			...
			Sleep(100);
			gotoxy(astX,astY); printf(" "); // I still have my doubts in this part, but it tries to signal a collision, sometimes the X remains theme...
