...
HealthBox::HealthBox(game* w, int hp, float x, float y, float dx, float dy) :CBasicEnemy(w, hp, x, y, dx, dy)
{
	tipe = BOX;
}
...
void HealthBox::Update(float elapsed)
{
	if (state == STATE_ACTIVE)
	{
		object::Update(elapsed);
