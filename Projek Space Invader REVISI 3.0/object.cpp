...
void CZigZagEnemy::Update(float elapsed)
{
	if (state == STATE_ACTIVE)
	{
		if (DX == 0.0f)
		{
			DX = rand() % 2 == 0 ? ZIGZAG_SPEED : -ZIGZAG_SPEED; // random arah awal
			Timer_Direction = SWITCH_TIMER;
		}

		if (this->GetLeft() <= 2.0f) // hit tembok kiri
		{
			this->SetX(2.0f);
			this->SetDX(ZIGZAG_SPEED);
			Timer_Direction = SWITCH_TIMER;
		}
		else if (this->GetRight() + 5.0f >= 75.0f) // hit tembok kanan
		{
			this->SetX(75.0f);
			this->SetDX(-ZIGZAG_SPEED);
			Timer_Direction = SWITCH_TIMER;
		}
		else // waktu belok / zig zag
		{
...
EnemyType CZigZagEnemy::GetType()
{
	return tipe;
}
...
EnemyType CSeekerEnemy::GetType()
{
	return tipe;
}

CHealthBox::CHealthBox(game* w, int hp, float x, float y, float dx, float dy) :CBasicEnemy(w, hp, x, y, dx, dy)
{
	tipe = BOX;
}

void CHealthBox::Draw()
{
	if (state == STATE_ACTIVE || state == STATE_DEADANIMATION && counter_die_anim % 2 == 1)
	{
...
void CHealthBox::Update(float elapsed)
{
	if (state == STATE_ACTIVE)
	{
		object::Update(elapsed);
...
EnemyType CHealthBox::GetType()
{
	return tipe;
}

CBomb::CBomb(game* w, int hp, float x, float y, float dx, float dy) :CBasicEnemy(w, hp, x, y, dx, dy)
{
	tipe = BOMB;
}
...
void CBomb::Update(float elapsed)
{
	if (state == STATE_ACTIVE)
	{
		object::Update(elapsed);
